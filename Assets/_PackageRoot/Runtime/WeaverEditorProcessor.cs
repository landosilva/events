...
[InitializeOnLoad]
public static class WeaverEditorProcessor
{
    static WeaverEditorProcessor()
    {
    ...
    private static void PatchAssembly(string assemblyPath)
    {
        ...
        string pkgPath = Path.GetFullPath(Path.Combine(Application.dataPath, "../Packages/com.lando.events/Runtime"));
        ...
        var eventBusType = ResolveEventBusType(module);
        if (eventBusType == null)
        {
            Debug.LogWarning("[Weaver] EventBus not found");
            module.Dispose();
            return;
        }

        var regDef   = eventBusType.Methods.FirstOrDefault(m => m.Name == "Register"   && m.HasGenericParameters && m.Parameters.Count == 1);
        var unregDef = eventBusType.Methods.FirstOrDefault(m => m.Name == "Unregister" && m.HasGenericParameters && m.Parameters.Count == 1);
    ...
    private static TypeDefinition ResolveEventBusType(ModuleDefinition module)
    {
        var found = module.Types.FirstOrDefault(t => t.Name == "EventBus");
        if (found != null) return found;

        foreach (var ar in module.AssemblyReferences.Where(r => r.Name.StartsWith("Lando.Events")))
        {
            AssemblyDefinition asm;
            try { asm = module.AssemblyResolver.Resolve(ar); }
            catch { continue; }
            found = asm.MainModule.Types.FirstOrDefault(t => t.Name == "EventBus");
            if (found != null) return found;
        }
        return null;
    }
